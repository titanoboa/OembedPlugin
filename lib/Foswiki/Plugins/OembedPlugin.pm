...
#
our $SHORTDESCRIPTION = 'third-party media embedding';;
...
sub initPlugin {
    ...
    # like this:
    # $Foswiki::cfg{Plugins}{OembedPlugin}{ExampleSetting} = 1;
    ...
    # LocalSite.cfg.
    # my $setting = $Foswiki::cfg{Plugins}{OembedPlugin}{ExampleSetting} || 0;

    # Register the _EXAMPLETAG function to handle %EXAMPLETAG{...}%
    # This will be called whenever %EXAMPLETAG% or %EXAMPLETAG{...}% is
    # seen in the topic text.
    Foswiki::Func::registerTagHandler( 'OEMBED', \&_OEMBED );
...
# ---++ StaticMethod _OEMBED($session, $params, $topic, $web, $topicObject) -> $string
# replaces the macro call of %OEMBED{...}%

sub _OEMBED {
    my($session, $params, $topic, $web, $topicObject) = @_;

    my $url = $params->{_DEFAULT} || $params->{url};
    return '%RED% No url given to be embedded %ENDCOLOR%' if !$url;

    my $provider_list = $Foswiki::cfg{Plugins}{OembedPlugin}{ProviderList} || [];
    my $maxwidth      = $params->{maxwidth} || $Foswiki::cfg{Plugins}{OembedPlugin}{Maxwidth} || 700;
    my $maxheight     = $params->{maxheight} || $Foswiki::cfg{Plugins}{OembedPlugin}{Maxheight} || 700;

    my $consumer = Web::oEmbed->new({ format => 'json' });

    foreach my $provider (@{$provider_list}){
	next if not $provider->{url};
	next if not $provider->{api};
	my $provider_to_register = { url => $provider->{url}, api => $provider->{api} };
	$provider_to_register->{name} = $provider->{name} if $provider->{name};
	$consumer->register_provider( $provider_to_register );
    }

    my $response = eval { $consumer->embed($url, {maxwidth => $maxwidth, maxheight => $maxheight}) };
